...
static int at76_set_tkip_bssid(struct at76_priv *priv, const void *addr)
{
	...
	at76_dbg_dump(DBG_MAC80211, addr, ETH_ALEN, "bssid:");
...
static void at76_dwork_hw_scan(struct work_struct *work)
{
	...
	/* ieee80211_scan_completed might call at76_config, which wants a lock */
	mutex_unlock(&priv->mtx);
...
static int at76_set_key_wpafw(struct ieee80211_hw *hw, enum set_key_cmd cmd,
			struct ieee80211_vif *vif, struct ieee80211_sta *sta, /*TODO: was local_address,address*/
			struct ieee80211_key_conf *key)
{
	...
	if (cmd == SET_KEY) {
		...
		at76_dbg_dump(DBG_PROGRESS, sta->addr, ETH_ALEN, "sta addr:");
		if (is_valid_ether_addr(sta->addr))
			/* Pairwise key */
			priv->mib_buf.data.data[39] |= (KEY_PAIRWISE | KEY_TX);
		else if (is_broadcast_ether_addr(sta->addr))
			/* Group key */
		...
		/* Set up default keys */
		if (is_broadcast_ether_addr(sta->addr))
			...
			struct ieee80211_key_conf *key)
{
	...
	if (FIRMWARE_IS_WPA(priv->fw_version))
		return at76_set_key_wpafw(hw,cmd,vif,sta,key);
